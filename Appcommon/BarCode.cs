...
public class BarCodeHelper
{
    ...
    public static string BarCode128(string InputData)
    {
        byte[] lb_start = Encoding.GetEncoding("ISO-8859-1").GetBytes("Ì");
        byte[] lb_stop = Encoding.GetEncoding("ISO-8859-1").GetBytes("Î");
        string ls_start = Encoding.GetEncoding("ISO-8859-1").GetString(new byte[] { (byte)204 });
        string ls_stop = Encoding.GetEncoding("ISO-8859-1").GetString(new byte[] { (byte)206 });
        if (InputData.Equals(""))
        {
	        return ls_start+"ErrorI"+ls_stop;
        }
        int li_check_digit = 0;
        for (int i = 1; i <= InputData.Length; i++)
        {
            string ls_char = InputData.Substring(i-1, 1);
            byte[] lb_char = Encoding.ASCII.GetBytes(ls_char);
            int li_value = Convert.ToInt16(lb_char[0])-32;
            li_check_digit = li_check_digit + li_value * i;
        }
        li_check_digit = (104 + li_check_digit)%103;
        if (li_check_digit==0)
        {
            li_check_digit = 32;
        }
        else if (li_check_digit<95)
        {
	        li_check_digit= li_check_digit +32 ;
        }
        else if (li_check_digit > 94)
        {
            li_check_digit = li_check_digit + 100;
        }
        byte[] lb_check = new byte[] { (byte)li_check_digit };
        string ls_check = Encoding.GetEncoding("ISO-8859-1").GetString(lb_check);
        return ls_start+InputData+ls_check+ls_stop;
    }
    /// <summary>
